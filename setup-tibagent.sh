...
function print_help() {
    ...
    echo -e "-a | --access-key: Optional, default is 'key'"
...
function start_tibagent() {
    ...
    echo -e "\nStep 3: Configuring connect for tibagent [$AGENT_NAME] with access key [$ACCESS_KEY] and secret key [$SECRET_KEY]..."
    fail_by_rc ./tibagent configure connect -a ${ACCESS_KEY} -s ${SECRET_KEY} $AGENT_NAME

    echo -e "\nStep 4: Starting tibagent [$AGENT_NAME] with spec [$SPEC]..."
